<template>
  <UModal>
    <div class="bg-[#0D0A0E] p-6 rounded-lg shadow-lg w-full h-full mx-auto">
      <h1 class="text-2xl font-bold text-white text-center mb-6">Edit Director</h1>
      <!-- Description Field -->
      <div class="mb-4">
        <label for="description" class="block text-sm text-white mb-2">Director</label>
        <textarea id="description" v-model="form.director"
          ...
          placeholder="director1,director2,..."></textarea>
      </div>
...
import { editDirector } from '~/composable/PUT/editDirector'
...
const save = async () => {
  if (!createArray(form.value.director).length) {
    alert("ValidateFail: Required director")
    emit('close')
    return
  }

  let Result = await editDirector(props.movieid, createArray(form.value.director))
